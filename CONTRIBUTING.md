...
{ lib, settingsLib, ... }:
let
	inherit (settingsLib) setting global bool inverted shows;
in
{
...
| `value` | `bool`, `inverted bool`, `text`, `strings`, `number { min; max; stored ? ; }`, `enum { "<label as shown>" = <stored value>; }` (`absent` deletes the key, `{ <key name> = …; }` writes several), `storedAs { true = 1; false = 0; } bool` (other stored values for a switch), `inDict "<entry>" <codec>` (one entry of a dictionary, the rest left alone), `dictSwitches`, `members` and `flags { <name> = <bit>; }` (named switches kept in a dictionary, an array or a bitmask; switches left out are kept), `hotKey <id>` (a keyboard shortcut, written as `"⌘⇧S"`; see `shortcuts.nix`). A codec that reads its keys back has `decode`; for `bool` and `enum` the value whose writes match is found without one. `nix flake check` fails when an option doesn't read back what it writes. | `codecs.nix` |
...
| `reads` | For state a service keeps rather than a preference (pmset, the firewall, sharing): `{ command; values ? null; parse ? null; }`, a command that prints the value, then either what it prints for each value, or a function from its JSON output to the value. `nix run .#current` runs it. | `setting.nix` |
| `verify` | What System Settings shows for each value: `expect = { <value> = { <control> = <shown>; }; }`, usually built with `shows.checkbox <control>`, `shows.choice <control> [ choices ]`, `shows.radio [ labels ]` or `shows.values <control> { <value> = <shown>; }`. `open` presses controls first; `operate` flips the control in the UI to check the option writes where System Settings reads. | `shows.nix` |
