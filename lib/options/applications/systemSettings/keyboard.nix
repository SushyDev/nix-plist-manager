...
# menu is in Menu Bar.
let
	inherit (settingsLib) setting global user bool storedAs enum number inDict hotKey activatesShortcuts;
	...
	shortcuts = { category, sidebar, group ? null, verifiable ? true }: rows: lib.mapAttrs (_: row: shortcut {
...
in
{
	...
	in {
		launchpadAndDock = shortcuts dock {
			turnDockHidingOnOff = [ "Turn Dock hiding on/off" 52 ];
		};

		display = shortcuts display {
		...
		missionControl = shortcuts missionControl {
		...
		} // shortcuts (inGroup missionControl 1 "Mission Control") {
		...
		} // shortcuts (inGroup windows 3 "Quarters") {
		...
		} // shortcuts (inGroup windows 4 "Arrange") {
		...
		} // shortcuts (inGroup windows 5 "Full Screen Tile") {
		...
		inputSources = shortcuts inputSources {
		...
		screenshots = shortcuts screenshots {
		...
		} // shortcuts (inGroupSwitchedAsOne accessibility 1 "Contrast") {
			increaseContrast = [ "Increase contrast" 25 ];
			decreaseContrast = [ "Decrease contrast" 26 ];
		} // shortcuts (inGroupSwitchedAsOne accessibility 2 "Live Captions") {
		...
		} // shortcuts (inGroupSwitchedAsOne accessibility 3 "Live Speech") {
